...
int isTmax(int x) 
{
  /*假设此时x为最大值，则x=0x7FFF FFFF ;  x+1=0xF000 0000 ;令a = x + 1 则x + a = 0xFFFF FFFF。二进制表示为全1.取反后为0
    ...
    特殊情况x=0xFFFF FFFF ;a = x+1 = 0x0000 0000 ; x+a=0xFFFF FFFF取反之后得到0x00000000 所以要排除x=0xFFFF FFFF*/
  int a = x + 1;
  x = a + x;
  x = ~x;
  a = !a;
...
{
  int m, n, z;
  m = x >> 31 ^ y >> 31;//x，y异号时m = 1
  n = !((x ^ ~y) + 1) | (((x + (~y + 1)) >> 31) & 1);//相等|x - y < 0
  z = (x >> 31) & 1;//判断x的符号，若为负则成立
  return (m & z) | (~m & n);//符号不同|符号相同
}
//4
/* 
...
{
  int y = ((~x + 1) | x);//x不为0时得到-1，x为0时得到0
  return (y >> 31)+ 1;//最后加1就得到答案
}
...
int howManyBits(int x) 
{
  //二分思想
  int y = x >> 31;
  x = (~y & x) | (y & ~x); // 如果x为正数保持原型，如果x为负数取反，使得非符号位最高位为1
...
int floatFloat2Int(unsigned uf) 
{
  ...
  if(real_exp >= 0)
  {
    if(real_exp >= 31)//overflow
      return 0x80000000;
    real_last_num = real_last_num >> (23 - real_exp);//舍弃小数部分
...
{
  if(x>127)//溢出
    return 0x7f800000;// 符号位0 阶码全1 尾数全0
    //规格化的 [-126,127]
  if(x>=-126)
  {
